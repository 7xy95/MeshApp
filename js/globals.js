...
const MAX_GET_REQUESTS = 15
const MAX_BLOCKS = 2048
const MAX_FBLOCKS = 32
const FEE_CHANGE_H = 23350
        ...
        if (tx.startsWith("SYSTEM|")) {
            ...
            if (tx.startsWith("MSG|")) {
                ...
                if (fee === undefined) {
                    ...
                    fee = Number(fee)
                    balancesCache[from] = (balancesCache[from]||0) - amount - 1 - fee
                    balancesCache[to] = (balancesCache[to]||0) + amount
                }
                nonceCache.add(`${from}|${nonce}`)
            }
        }
...
const nodesPath = path.join(dataDir, "savedPeers.json")
const blocksPath = path.join(dataDir, "blocks.json")
const session = path.join(dataDir, "session.json")
...
global.MAX_GET_REQUESTS = MAX_GET_REQUESTS
global.MAX_BLOCKS = MAX_BLOCKS
global.MAX_FBLOCKS = MAX_FBLOCKS
global.FEE_CHANGE_H = FEE_CHANGE_H
...
global.miningAddress = miningAddress

global.allNodes = allNodes
global.activeNodes = activeNodes
global.MAX_ACTIVE_NODES = MAX_ACTIVE_NODES

global.difficultyCache = difficultyCache
global.balancesCache = balancesCache
global.nonceCache = nonceCache
global.cacheBlock = cacheBlock

global.contacts = contacts
global.nodesPath = nodesPath
global.blocksPath = blocksPath
global.session = session

global.allIds = allIds
global.idsInfo = idsInfo

global.batteryLevel = batteryLevel
global.batteryThreshold = batteryThreshold
global.forkCase = forkCase
global.forkBlockStart = forkBlockStart
global.forkData = forkData
global.mine = mine
global.useGPU = useGPU
global.throttleTime = throttleTime
global.minBattery = minBattery
global.batchSize = batchSize

global.totalHashes = totalHashes
global.totalHashesFound = totalHashesFound
...
global.noStart = noStart
global.idRequestCount = idRequestCount
global.blacklistedIds = blacklistedIds
